    ...
    try:
        ...
        match metod:
            case "add_item":    
                ...
                todo[r["index"]]["created"] = datetime.now().strftime('%Y-%m-%d %H:%M')

            case "add_list":    todo.append({"name": r["name"], "list": []})
            case "move_item":   todo[r["list"]]["lists"]["list"].insert(r["new_index"], 
                                todo[r["list"]]["lists"]["list"].pop(r["old_index"]))
            case "move_list":   todo.insert(r["new_index"], todo.pop(r["old_index"]))
            case "remove_item": del todo[r["list_index"]]["lists"]["list"][r["item_index"]]
            case "remove_list": del todo[r["index"]]
            case "rename_item": todo[r["list_index"]]["lists"]["list"][r["item_index"]] = r["name"]
